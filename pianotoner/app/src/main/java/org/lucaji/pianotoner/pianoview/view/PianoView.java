package org.lucaji.pianotoner.pianoview.view;
...
import org.lucaji.pianotoner.R;
import org.lucaji.pianotoner.pianoview.entity.AutoPlayEntity;
import org.lucaji.pianotoner.pianoview.entity.Piano;
import org.lucaji.pianotoner.pianoview.entity.PianoKey;
...
import org.lucaji.pianotoner.pianoview.utils.AudioUtils;
...
public class PianoView extends View {
  ...
  @Override protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    //最小高度
    if (whiteKeyDrawable == null) { return; }
    ...
    if (whitePianoKeys != null) {
      for (int i = 0; i < whitePianoKeys.size(); i++) {
        for (PianoKey key : whitePianoKeys.get(i)) {
          ...
          square.set(left, top-100, right, bottom-100);
          paint.setShadowLayer(1.0f,1.0f,1.0f,Color.BLACK);
          canvas.drawCircle(square.centerX(), square.centerY(), (float) (sideLength/2.0), paint);
    ...
    if (canPress) {
      switch (event.getAction()) {
        ...
        case MotionEvent.ACTION_MOVE:
          ...
          break;
        //多点触控，当其他手指点击键盘的手
        //case MotionEvent.ACTION_POINTER_DOWN: // MARKED AS DUPLICATE WITH ACTION_DOWN
        //  handleDown(event.getActionIndex(), event);
        //  break;
        //多点触控，当其他手指抬起的时候
  ...
  private void handleMove(int which, MotionEvent event) {
    ...
    for (PianoKey key : pressedKeys) {
      if (key.getFingerID() == event.getPointerId(which)) {
        if (!key.contains(x, y)) {
          key.getKeyDrawable().setState(new int[] { -android.R.attr.state_pressed });
          // invalidate(key.getKeyDrawable().getBounds()); // Deprecated API28 not need of dirtyRect anymore https://stackoverflow.com/questions/52647158/updating-only-a-portion-of-view-without-using-invalidaterect
          invalidate();
